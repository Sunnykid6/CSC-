...
public class GameBoard {
    ...
    public String isWord(String word) {
        ...
        for (int i = 0; i < NUM_ROWS; i++) {
            for (int j = 0; j < NUM_COLS; j++) {
                if (board[i][j] == wordArray[theStack.size()] && !(visited[i][j])) {
                    theStack.push("(" + i + ", " + j + ")");
                    visited[i][j] = true;
                    if (board[i][j] != wordArray[0]) {
                        return null;
                    }
                    if (findPath(word, theStack, i, j)) {
                        String result = theStack.toString();
                        return result;
                    } 
					else {
                        theStack.pop();
    ...
    private boolean findPath(String wordToFind, Stack<String> pathSoFar, int row, int col) {
        ...
        char letter = wordArray[theStack.size()];
        for (int r = row - 1; r < row + 2; r++) {
            for (int c = col - 1; c < col + 2; c++) {
                if ((r != -1 || c != -1) && (r != -1 && c != -1) && (r != 4 || c != 4) && (r != 4 && c != 4)) {
                    if ((board[r][c] == letter) && (visited[r][c] != true)) {
                        pathSoFar.push("(" + r + ", " + c + ")");
                        visited[r][c] = true;
                        if (theStack.size() == wordToFind.length()) {
                            return true;
                        } 
						else if (findPath(wordToFind, theStack, r, c)) {
                            return true;
                        } 
						else {
                            if (!(theStack.isEmpty())) {
                                theStack.pop();
                                return findPath(wordToFind, theStack, row, col);
                            }
                        }
                    }
                }
    ...
    static public void main(String[] args) {
        ...
        GameBoard test01 = new GameBoard(123);
